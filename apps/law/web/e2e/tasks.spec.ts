/**
 * The task working-day flow (T04b.2 gate): create a task against a real
 * case by typing its court case number, watch the derived facts arrive
 * (case number on the row — D9; overdue — the record model), drive the
 * lifecycle through updateStatus, converse in comments, and verify the
 * "My Tasks" contract keeps other people's tasks off Home.
 */
...
test("create → detail → status → comment → home, with derived facts throughout", async ({ page }) => {
  await signIn(page, ASHA.email, ASHA.password);

  // Standalone creation: the lawyer types the court case number.
  ...
  // Detail: derived case number, Open state, overdue badge.
  await expect(page.getByRole("heading", { name: "Draft rejoinder" })).toBeVisible();
  await expect(page.getByRole("link", { name: SEED_CASE.caseNumber })).toBeVisible();
  ...
  // Home: my task, at a glance — case number, overdue, state.
  ...
  await expect(row).toContainText(SEED_CASE.caseNumber);
  await expect(row).toContainText("Overdue");
  await expect(row).toContainText("In progress");
});

test("'My Tasks' is the caller's view: a colleague's task stays off my home", async ({ page }) => {
  await signIn(page, ASHA.email, ASHA.password);
  ...
  // Not on Asha's home (the contract's default is HER assignments) …
  await page.goto("/");
  await expect(page.getByRole("heading", { name: /Welcome/ })).toBeVisible();
