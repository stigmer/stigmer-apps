/**
 * Channel-identity headers → ChannelIdentity, plus the privacy-safe log
 ...
 */
export function channelIdentityFromHeaders(req: IncomingMessage): ChannelIdentity | undefined {
 ...
 * a phone number — personal data that never enters a log line (and the
 * repo's CI guard would fail on a real-shaped number anyway); the hash
 * prefix lets an operator correlate one caller's requests without
 * learning who they are.
 */
export function identityLogToken(identity: ChannelIdentity | undefined): string {
